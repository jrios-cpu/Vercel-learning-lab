...
export async function POST(req: NextRequest) {
  ...
  if (!(["configurator", "quick_rfq"] as const).includes(source as "configurator" | "quick_rfq")) return json({ error: "Select a valid quote source.", requestId }, 400);
  ...
  const selectedProducts = partNumbers.map((partNumber) => PRODUCTS.find((product) => product.partNumber === partNumber)).filter(Boolean);
  if (!partNumbers.length || partNumbers.length > 20 || selectedProducts.length !== partNumbers.length) return json({ error: "Select valid representative products.", requestId }, 400);
  ...
  if (source === "configurator") {
    ...
    if (partNumbers.length !== 1) return json({ error: "Quick quote requires exactly one product.", requestId }, 400);
    const product = selectedProducts[0]!;
    if (productLine !== product.series) return json({ error: "Quick quote product line does not match the selected product.", requestId }, 400);
    if (notes.length < 8) return json({ error: "Project notes must be between 8 and 3000 characters.", requestId }, 400);
  }

  const quoteRef = `RXL-${randomUUID().slice(0, 8).toUpperCase()}`;
  const responseBody = { ok: true, quoteRef, delivery: "not-configured", requestId };
  console.info("rxl_quote_request", { requestId, quoteRef, source, partCount: partNumbers.length });
