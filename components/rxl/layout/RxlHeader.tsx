...
export function RxlHeader({ preview }: { preview: boolean }) {
  ...
  return (
    <>
      {preview && (
        <div className="rxl-preview" role="status">
          <strong>RXL Preview</strong>
          <span>Representative content. External integrations are not live.</span>
        </div>
      )}
        ...
        <div className="rxl-header-inner">
          ...
          <div className="rxl-header-actions">
            {RXL_SITE.contact.phone && (
              <a className="rxl-phone" href={`tel:${RXL_SITE.contact.phone.replace(/[^+\d]/g, "")}`}>
                {RXL_SITE.contact.phone}
              </a>
            )}
            ...
            <button
              className={`rxl-burger${mobileOpen ? " open" : ""}`}
              type="button"
              aria-label="Menu"
              aria-expanded={mobileOpen}
              aria-controls="rxl-mobile-nav"
