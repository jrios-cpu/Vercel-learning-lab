...
export function Configurator({ products, initialProduct }: { products: Product[]; initialProduct?: Product | null }) {
  ...
  const next = () => { if (!canAdvance(state)) return; trackAnalytics("configurator_step", { step: state.step, direction: "next" }); dispatch({ type: "NEXT" }); };
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const attribution = { source: "direct", medium: null, campaign: null, landingPage: window.location.pathname + window.location.search, sessionId: window.crypto.randomUUID() };
      const payload = { ...buildQuotePayload(state, { name: contact.name.trim(), email: contact.email.trim(), company: contact.company.trim() || null }, attribution), website };
  ...
  return (
    <form className="rxl-configurator" onSubmit={submit}>
      <div className="rxl-config-main">
        <ol className="rxl-config-steps" aria-label="Configurator progress">{CONFIGURATOR_STEPS.map((label, index) => <li className={state.step === index + 1 ? "active" : state.step > index + 1 ? "done" : ""} key={label}><span>{index + 1}</span>{label}</li>)}</ol>
        <section className="rxl-config-panel">
          <span className="rxl-eyebrow">Step {state.step} of 5</span>
          {state.step === 1 && <><h2>Where will this system work?</h2><div className="rxl-choice-grid">{APPLICATIONS.map((application) => <label key={application}><input type="radio" name="application" value={application} checked={state.application === application} onChange={() => dispatch({ type: "SET_APPLICATION", value: application })} /><span>{application}</span></label>)}</div></>}
