...
import { CONFIGURATOR_STEPS, canAdvance, configuratorReducer, initialConfiguratorState, type ConfiguratorState } from "./configuratorReducer";
...
const STORAGE_KEY = "rxl-configurator-rc1";
type ContactState = { name: string; email: string; company: string };
type StoredConfigurator = { state: ConfiguratorState; contact: ContactState };
...
export function Configurator({ products, initialProduct }: { products: Product[]; initialProduct?: Product | null }) {
  ...
  const [contact, setContact] = useState<ContactState>({ name: "", email: "", company: "" });
  ...
  useEffect(() => {
    trackAnalytics("configurator_start", { source: initialProduct ? "product" : "direct" });
    const frame = window.requestAnimationFrame(() => {
      if (!initialProduct) {
        try {
          const raw = window.sessionStorage.getItem(STORAGE_KEY);
          if (raw) {
            const stored = JSON.parse(raw) as Partial<StoredConfigurator>;
            if (stored.state) dispatch({ type: "RESTORE", state: stored.state });
            if (stored.contact) setContact({ name: stored.contact.name ?? "", email: stored.contact.email ?? "", company: stored.contact.company ?? "" });
          }
        } catch {
  ...
  useEffect(() => {
    if (!hydrated) return;
    const stored: StoredConfigurator = { state, contact };
    window.sessionStorage.setItem(STORAGE_KEY, JSON.stringify(stored));
  }, [contact, hydrated, state]);
  ...
  const goTo = (step: ConfiguratorState["step"]) => { if (step <= state.step) { trackAnalytics("configurator_step", { step, direction: "edit" }); dispatch({ type: "GO_TO", step }); } };
  ...
  return (
    <form className="rxl-configurator" onSubmit={submit}>
      <div className="rxl-config-main">
        <ol className="rxl-config-steps" aria-label="Configurator progress">
          {CONFIGURATOR_STEPS.map((label, index) => {
            const step = (index + 1) as ConfiguratorState["step"];
            return <li className={state.step === step ? "active" : state.step > step ? "done" : ""} key={label}><button type="button" disabled={step > state.step} aria-current={state.step === step ? "step" : undefined} onClick={() => goTo(step)}><span>{step}</span>{label}</button></li>;
          })}
        </ol>
        <section className="rxl-config-panel">
          <span className="rxl-eyebrow">Step {state.step} of 5</span>
          {state.step === 1 && <><h2>Where will this system work?</h2><div className="rxl-choice-grid">{APPLICATIONS.map((application) => <label key={application}><input type="radio" name="application" checked={state.application === application} onChange={() => dispatch({ type: "SET_APPLICATION", value: application })} /><span>{application}</span></label>)}</div></>}
          {state.step === 2 && <><h2>Select a product line.</h2><label className="rxl-field"><span>Product line</span><select value={state.productLine} onChange={(event) => dispatch({ type: "SET_PRODUCT_LINE", value: event.target.value })}><option value="">Choose a line</option>{lines.map((line) => <option key={line}>{line}</option>)}</select></label><label className="rxl-field"><span>Representative product</span><select value={state.partNumbers[0] ?? ""} disabled={!state.productLine} onChange={(event) => dispatch({ type: "SET_PART_NUMBER", value: event.target.value })}><option value="">Choose a product</option>{lineProducts.map((product) => <option value={product.partNumber} key={product.partNumber}>{product.partNumber} — {product.title}</option>)}</select></label></>}
          {state.step === 3 && <><h2>Set the working configuration.</h2><label className="rxl-field"><span>Finish / option summary</span><input maxLength={120} value={state.selections.finish ?? selectedProduct?.finish ?? ""} onChange={(event) => dispatch({ type: "SET_SELECTION", key: "finish", value: event.target.value })} /></label><label className="rxl-field"><span>Quantity</span><input type="number" min="1" max="100000" value={state.quantity} onChange={(event) => dispatch({ type: "SET_QUANTITY", value: Number(event.target.value) })} /></label><p className="rxl-config-hint">Configuration fields are representative until the Epicor/Sanity product model is connected.</p></>}
          {state.step === 4 && <><h2>Add project accessories.</h2><div className="rxl-choice-grid">{ACCESSORIES.map((accessory) => <label key={accessory}><input type="checkbox" checked={state.accessories.includes(accessory)} onChange={() => dispatch({ type: "TOGGLE_ACCESSORY", value: accessory })} /><span>{accessory}</span></label>)}</div><p className="rxl-config-hint">Accessories are optional representative choices.</p></>}
          {state.step === 5 && <><h2>Project details.</h2><div className="rxl-form-grid"><label className="rxl-field"><span>Name</span><input required maxLength={80} value={contact.name} onChange={(event) => setContact({ ...contact, name: event.target.value })} /></label><label className="rxl-field"><span>Work email</span><input required type="email" maxLength={254} value={contact.email} onChange={(event) => setContact({ ...contact, email: event.target.value })} /></label><label className="rxl-field"><span>Company</span><input maxLength={120} value={contact.company} onChange={(event) => setContact({ ...contact, company: event.target.value })} /></label><label className="rxl-field"><span>Target timeline</span><input maxLength={120} value={state.targetTimeline} onChange={(event) => dispatch({ type: "SET_TIMELINE", value: event.target.value })} /></label><label className="rxl-field rxl-field-wide"><span>Project notes</span><textarea maxLength={3000} rows={5} value={state.notes} onChange={(event) => dispatch({ type: "SET_NOTES", value: event.target.value })} /></label><label className="rxl-honeypot" aria-hidden="true">Website<input tabIndex={-1} autoComplete="off" value={website} onChange={(event) => setWebsite(event.target.value)} /></label></div><div className="rxl-config-api-note">Preview acknowledgment only. No CRM, email delivery, pricing, or inventory workflow is connected.</div></>}
          <div className="rxl-config-nav"><button className="rxl-btn rxl-btn-outline" type="button" disabled={state.step === 1} onClick={previous}>Back</button>{state.step < 5 ? <button className="rxl-btn rxl-btn-primary" type="button" disabled={!canAdvance(state)} onClick={next}>Continue</button> : <button className="rxl-btn rxl-btn-primary" type="submit" disabled={busy}>{busy ? "Submitting…" : "Submit project request"}</button>}</div>
          <p className="rxl-form-status" role="status" aria-live="polite">{status}</p>
        </section>
      </div>
      <aside className="rxl-config-summary" aria-label="Configuration summary">
        <div className="rxl-config-summary-title"><span>Project summary</span>{hydrated && <span className="rxl-config-persist">Saved in this browser</span>}</div>
        <h3>{state.productLine || "Product line pending"}</h3>
        {selectedProduct && <span className="rxl-selected-product-name">{selectedProduct.title}</span>}
        <dl><div><dt>Application</dt><dd>{state.application || "—"}</dd></div><div><dt>Part number</dt><dd>{state.partNumbers[0] || "—"}</dd></div><div><dt>Quantity</dt><dd>{state.quantity}</dd></div><div><dt>Accessories</dt><dd>{state.accessories.length || "—"}</dd></div></dl>
        <div className="rxl-summary-actions"><button type="button" onClick={() => goTo(1)}>Edit application</button><button type="button" disabled={state.step < 2} onClick={() => goTo(2)}>Edit product</button><button type="button" disabled={state.step < 3} onClick={() => goTo(3)}>Edit configuration</button><button type="button" disabled={state.step < 4} onClick={() => goTo(4)}>Edit accessories</button></div>
        <p>Representative Preview configuration. Commercial validation happens only after real systems are connected.</p>
      </aside>
    </form>
  );
}
